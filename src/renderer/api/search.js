...
async function getAlbums(keywords, offset = 0) {
    try {
        ...
        if (res.data.code === 200) {
            return res.data.result.albums.map(e => {
                const { artist } = e;
                return {
                    id: e.id,
                    name: e.name,
...
async function getArtists(keywords, offset = 0) {
    try {
        ...
        if (res.data.code === 200) {
            return res.data.result.artists.map(e => {
                return {
                    id: e.id,
                    name: e.name,
                    avatar:
...
async function getUsers(keywords, offset = 0) {
    try {
        ...
        if (res.data.code === 200) {
            return res.data.result.userprofiles.map(e => {
                return {
                    id: e.userId,
                    name: e.nickname,
                    avatar: e.avatarUrl,
...
async function searchByType(type, keyword, offset) {
    let data;
