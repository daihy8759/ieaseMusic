...
async function getRecentUser(id) {
    try {
        ...
        if (res.data.code === 200) {
            return res.data.userprofiles.map(e => {
                return {
                    id: e.userId,
                    name: e.nickname,
...
async function getSimilarPlaylist(id) {
    try {
        ...
        if (res.data.code === 200) {
            return res.data.playlists.map(e => {
                return {
                    id: e.id,
                    name: e.name,
                    cover: e.coverImgUrl,
...
async function getRecommend(songid, artistid) {
...
async function getPlayListDetail(type, id) {
    const resData = {};
    let songs = [];
    if (type === '0') {
